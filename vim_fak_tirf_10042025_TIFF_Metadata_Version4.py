...
tiff_path = r'C:\Users\Arun\Desktop\TIRF MCHERRYVIM_GFPFAK_TIFF\MEF KO GFP FAK MCHERRY VIM G.tiff'
...
output_dir = r'C:/Users/Arun/Downloads/PhD/Manuscript/TIRF LIVE IMAGING/Python Analysis Results/'
...
# [Callback function code remains the same - omitted for brevity]
...
if use_interactive_roi:
    ...
    try:
        ...
        # <<< CHANGE HERE: Select FAK channel (axis 0 of the slice) for display >>>
        print("Using FAK channel for ROI selection display.")
        display_channel = first_time_frame_multi_channel[fak_channel_index, :, :] # Shape: (Y, X)
        # <<< END CHANGE >>>
    ...
    print("INSTRUCTIONS:")
    # <<< CHANGE HERE: Update instructions >>>
    print(" - Using first time frame (FAK channel) for selection.")
    # <<< END CHANGE >>>
...
# [Data integrity check code remains the same - omitted for brevity]
...
# --- NORMALIZATION ---
# [Normalization code remains the same - omitted for brevity]
...
# --- SMOOTHING ---
# [Smoothing function code remains the same - omitted for brevity]
...
for i in range(num_rois):
    ...
    if i < len(vimentin_intensity) and i < len(fak_intensity) and \
       ...
       i < len(fak_area):
        ...
        plot_filename = f"ROI_{i+1}_Intensity_Area_Plot.png"
        plot_output_path = os.path.join(output_dir, plot_filename)

        # <<< Save the plot >>>
        try:
            plt.savefig(plot_output_path, dpi=150) # Save before showing
            print(f"Plot saved to {plot_output_path}")
        except Exception as e:
            print(f"Error saving plot {plot_output_path}: {e}")

        # <<< Show the plot (optional) >>>
        # plt.show() # Uncomment this if you still want to see the plot interactively
...
# Calculate time points using the determined interval
...
try:
    ...
    # >>> Construct CSV output path using output_dir <<<
    csv_output_filename = 'FAK_VIM_ROI_Tracking_FROM_TIFF_TCYX_autoFPS.csv' # Suffix indicates format and auto-fps attempt
    csv_output_path = os.path.join(output_dir, csv_output_filename)

    # Directory creation is already done near the beginning
...
if not export_successful:
    try:
        ...
        fallback_path = os.path.join(output_dir, 'FALLBACK_RAW_DATA_FROM_TIFF_TCYX_autoFPS.json') # Changed suffix
        import json
