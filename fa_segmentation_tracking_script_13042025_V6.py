...
output_dir = r'C:/Users/Arun/Downloads/PhD/Manuscript/TIRF LIVE IMAGING/Python Analysis Results/Trial/Trail 2'
...
cfg_add_opening = True       # <<< Added flag for opening step >>>
cfg_opening_disk_radius = 1  # <<< Radius for opening disk >>>
cfg_add_closing = True       # <<< Added flag for closing step >>>
...
def clean_mask(mask, min_area=10, max_area=None, remove_border=True,
               add_opening=True, opening_radius=1, add_closing=True, closing_radius=2): # <<< Added morph params
    ...
    # Optional: Remove large objects (applied AFTER opening/closing)
    if max_area is not None and max_area > 0:
         labels, num_labels = measure.label(cleaned_mask, return_num=True, connectivity=2) # Use connectivity=2
         if num_labels > 0:
    ...
    # Optional: Remove objects touching the border (applied last)
    if remove_border:
        if np.any(cleaned_mask):
...
# It will still tune sigma, min_area, local_block, local_offset
# The opening/closing radii are currently fixed from config, but could be added as trackbars if needed.
def interactive_parameter_tuning(full_image_stack, fak_ch_index, num_total_frames, initial_params):
    ...
    print("Use 'Key Frame' slider to switch preview (0=Start, 1=Mid, 2=Break, 3=End).") # Updated label
    ...
    # Add frame near previous break point
    break_frame_idx = min(last_frame_idx, 217) # Frame where track 0 ended before
    ...
    while True:
        ...
        if key_frame_slider_pos != last_key_frame_slider_pos or params_changed:
            ...
            try:
                ...
                cleaned_mask = clean_mask(binary_mask,
                                          ...
                                          add_opening=cfg_add_opening, # Use config value
                                          opening_radius=cfg_opening_disk_radius,
                                          add_closing=cfg_add_closing, # Use config value
                                          closing_radius=cfg_closing_disk_radius)
                ...
                text_y = 20
                cv2.putText(preview_img, f"Preview: {frame_label_str}", (10, text_y), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA); text_y += 25 # White text
                ...
                last_key_frame_slider_pos = key_frame_slider_pos # Update slider tracker
        ...
        if key == 27: # Esc key
            print("\nParameter tuning cancelled by user. Exiting.")
            cv2.destroyAllWindows(); sys.exit()
        elif key == 13: # Enter key
            print("\nParameters accepted by user.")
            break # Exit loop

    cv2.destroyAllWindows()
    for i in range(5): cv2.waitKey(1)
    # Return the parameters as they were when Enter was pressed
    # Also include fixed params from initial_params that weren't tunable here
    final_params = initial_params.copy()
    final_params.update(current_params) # Overwrite tunable ones
    return final_params
    ...
    while True:
        ...
        if frame_idx != last_frame_idx:
            ...
            try:
                ...
                # Use approved cleaning parameters
                cleaned_mask = clean_mask(binary_mask,
                ...
                preview_img = cv2.cvtColor(display_img_8u, cv2.COLOR_GRAY2BGR)
                contours, _ = cv2.findContours(cleaned_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                cv2.drawContours(preview_img, contours, -1, (0, 255, 0), 1) # Green contours
        ...
        if preview_img is not None:
            cv2.imshow(window_name, preview_img)
        else: # Should only happen if frame 0 fails
             img_shape = full_image_stack.shape
             blank_img = np.zeros((img_shape[2], img_shape[3], 3), dtype=np.uint8)
        ...
        if key == 27: # Esc key
            print("\nSegmentation preview aborted by user. Exiting.")
            cv2.destroyAllWindows()
            return False # Indicate cancellation
        elif key == 13: # Enter key
            print("\nSegmentation preview approved by user. Continuing analysis.")
            break # Exit loop
...
def main():
    ...
    # <<< Read morph cleaning params from config >>>
    ...
    initial_params = {
        ...
        # <<< Pass morph params (although not tuned interactively here) >>>
    ...
    # Update other params from the returned dict (even if not tuned, keeps consistency)
    ...
    for t in range(num_frames):
        ...
        # Use approved parameter variables from tuning
        ...
        # Pass approved morph params to clean_mask
    ...
    # --- Step 4: Combine features from all frames ---
    if not all_features:
    ...
    if tracks_filtered.empty:
        ...
        analysis_parameters = {
            'Input File': tiff_path,
            'Output Directory': output_dir,
            'Output Prefix': output_prefix,
            'Vimentin Channel Index': vimentin_channel_index,
            'FAK Channel Index': fak_channel_index,
            'Frame Interval (sec)': frame_interval_sec,
            'Frame Rate (FPS)': frame_rate,
            '--- Segmentation ---': '--- TUNED PARAMETERS ---',
            'Segmentation Method': param_segmentation_method,
            'Use Gaussian Blur': param_use_gaussian_blur,
            ...
            'Min FA Area': param_min_fa_area,
            'Max FA Area': param_max_fa_area,
            'Remove Border Objects': param_remove_border_objects,
            'Use Opening': param_add_opening,
            'Opening Radius': param_opening_radius,
            'Use Closing': param_add_closing,
            'Closing Radius': param_closing_radius,
            '--- Tracking ---': '--- FIXED PARAMETERS ---',
            'Search Range (pixels)': cfg_search_range,
            'Memory (frames)': cfg_memory,
        ...
        try:
            ...
            print(f"Error saving parameters to {param_log_path}: {e}")
        # <<< END ADDED >>>
