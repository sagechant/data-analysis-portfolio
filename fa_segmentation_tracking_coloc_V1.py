...
output_dir = r'C:/Users/Arun/Downloads/PhD/Manuscript/TIRF LIVE IMAGING/Python Analysis Results/Coloc Trail 1'
...
def main():
    ...
    for t in range(num_frames):
        ...
        if num_labels > 0:
            ...
            for i, prop_f in enumerate(props_fak):
                prop_v = props_vim[i] # Corresponding vimentin properties
                ...
                try:
                    ...
                    if coords.shape[0] > 1:
                        fak_pixels = fak_frame[coords[:, 0], coords[:, 1]]
                        vim_pixels = vimentin_frame[coords[:, 0], coords[:, 1]]
                        if np.std(fak_pixels) > 1e-6 and np.std(vim_pixels) > 1e-6:
                             pcc = np.corrcoef(fak_pixels, vim_pixels)[0, 1]
                except Exception: pass

                # Calculate Manders' Coefficients (M1, M2)
                m1_coeff, m2_coeff = np.nan, np.nan # Defaults
                try:
                    coords = prop_f.coords
                    if coords.shape[0] > 0:
                        fak_pixels = fak_frame[coords[:, 0], coords[:, 1]]
                        ...
                        if pd.notna(vim_thresh) and pd.notna(fak_thresh):
                            vim_signal_mask = vim_pixels > vim_thresh
                            fak_signal_mask = fak_pixels > fak_thresh
                            sum_vim_total = np.sum(vim_pixels); sum_fak_total = np.sum(fak_pixels)
                            sum_vim_coloc = np.sum(vim_pixels[fak_signal_mask])
                            sum_fak_coloc = np.sum(fak_pixels[vim_signal_mask])
                ...
                # <<< ADDED: Calculate Intensity Correlation Quotient (ICQ) >>>
                icq = np.nan # Default
                try:
                    coords = prop_f.coords
                    if coords.shape[0] > 1:
                        fak_pixels = fak_frame[coords[:, 0], coords[:, 1]]
                        vim_pixels = vimentin_frame[coords[:, 0], coords[:, 1]]
                        mean_fak = np.mean(fak_pixels)
                        mean_vim = np.mean(vim_pixels)
                        # Calculate product of differences from mean
                        products = (fak_pixels - mean_fak) * (vim_pixels - mean_vim)
                        # Count positive products
                        num_positive = np.sum(products > 0)
                        total_pixels = len(products)
                        if total_pixels > 0:
                            icq = (num_positive / total_pixels) - 0.5
                except Exception as icq_e:
                    # print(f"\nWarning: Could not calculate ICQ for label {label} frame {t}: {icq_e}") # Optional
                    pass # Keep icq as NaN
                # <<< END ICQ Calculation >>>

                features_dict = {
                    'label': label, 'centroid-0': centroid[0], 'centroid-1': centroid[1],
                    'area_pixels': area_pix, 'bbox-0': bbox[0], 'bbox-1': bbox[1], 'bbox-2': bbox[2], 'bbox-3': bbox[3],
                    'fak_intensity_mean': fak_mean, 'fak_intensity_max': fak_max, 'fak_intensity_integrated': fak_integ,
                    'vimentin_intensity_mean': vim_mean, 'vimentin_intensity_integrated': vim_integ,
                    'pcc': pcc,
                    'm1_coeff': m1_coeff,
                    'm2_coeff': m2_coeff,
                    'icq': icq, # Add ICQ
                    'frame': t, 'y': centroid[0], 'x': centroid[1]
                }
            ...
            if frame_features_list:
                features_df = pd.DataFrame(frame_features_list)
    ...
    if tracks_filtered.empty:
        ...
        analysis_parameters = {
            # ... (parameter logging dictionary content remains the same, but now includes PCC, MOC, ICQ description) ...
            ...
            'Colocalization Metrics': 'Pearson Correlation (PCC), Manders Overlap (M1, M2), ICQ' # <<< Updated description
        }
        try:
        ...
        # Trajectory Plot [remains the same - omitted]
        ...
        try:
            ...
            traj_plot_path = os.path.join(output_dir, f"{output_prefix}_trajectories.png"); fig_traj.savefig(traj_plot_path, dpi=150); print(f"Trajectory plot saved to {traj_plot_path}")
        ...
        if num_plots_to_show > 0:
             ...
             for i in range(num_plots_to_show):
                  ...
                  # <<< MODIFIED: Create 9 subplots >>>
                  fig_ind, axes = plt.subplots(9, 1, figsize=(10, 20), sharex=True) # Increased height further
                  ...
                  # Plot PCC
                  axes[4].plot(time_sec, track_data['pcc'], 'k.-', label='PCC (Vim/FAK)')
                  axes[4].set_ylabel('Pearson Corr Coeff'); axes[4].legend(loc='upper left'); axes[4].grid(True); axes[4].set_ylim(-1.1, 1.1)
                  # Plot M1
                  axes[5].plot(time_sec, track_data['m1_coeff'], 'y.-', label='M1 (Vim Overlap FAK)')
                  axes[5].set_ylabel('Manders M1'); axes[5].legend(loc='upper left'); axes[5].grid(True); axes[5].set_ylim(-0.1, 1.1)
                  # Plot M2
                  axes[6].plot(time_sec, track_data['m2_coeff'], 'b.-', label='M2 (FAK Overlap Vim)') # Changed color
                  axes[6].set_ylabel('Manders M2'); axes[6].legend(loc='upper left'); axes[6].grid(True); axes[6].set_ylim(-0.1, 1.1)

                  # <<< ADDED: Plot ICQ >>>
                  axes[7].plot(time_sec, track_data['icq'], color='orange', marker='.', linestyle='-', label='ICQ (Vim/FAK)')
                  axes[7].set_ylabel('ICQ'); axes[7].legend(loc='upper left'); axes[7].grid(True); axes[7].set_ylim(-0.6, 0.6) # Range -0.5 to 0.5

                  # Plot Area (use correct column and label) - Now axes[8]
                  area_col = 'area_um2' if area_per_pixel_um2 is not None and 'area_um2' in track_data.columns else 'area_pixels'
                  axes[8].plot(time_sec, track_data[area_col], 'b.-', label='Area') # Changed color back to blue
                  axes[8].set_ylabel(f'Area ({area_units_label})') # Use dynamic label
                  axes[8].legend(loc='upper left'); axes[8].grid(True)
                  axes[8].set_xlabel(x_label + f' (Frame Rate approx. {frame_rate:.3f} FPS)')
                  # <<< END MODIFICATIONS >>>

                  plt.tight_layout(rect=[0, 0.03, 1, 0.98]) # Adjust layout slightly for 9 plots
