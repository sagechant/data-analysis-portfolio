...
output_dir = r'C:\Users\Arun\Desktop\TIRF MCHERRYVIM_GFPFAK_TIFF\Analysis Trail'
...
cfg_pixel_size_um = 0.08 # <<< SET MANUALLY ONLY IF NEEDED >>>
...
def main():
        ...
        with tifffile.TiffFile(tiff_path) as tif:
                ...
                if ij_meta:
                    ...
                    if 'unit' in ij_meta and ij_meta['unit'] in ('micron', 'um'):
                        if 'pixel_width' in ij_meta and 'pixel_height' in ij_meta:
                             if ij_meta['pixel_width'] == ij_meta['pixel_height']: # Assume square pixels
                                 if pixel_size_um is None: # Only use if not manually set
                                     pixel_size_um = float(ij_meta['pixel_width'])
                                     print(f"  Read pixel size from ImageJ metadata: {pixel_size_um:.4f} um/pixel")
                                 else: print("  Manual pixel size set, ignoring ImageJ pixel size.")
                             else: print("  Warning: ImageJ metadata has non-square pixels, cannot determine single pixel size.")
                    if 'finterval' in ij_meta:
            ...
            try:
                ...
                if ome_xml and isinstance(ome_xml, str):
                    print("Found OME metadata string, attempting to parse...")
                    root = ET.fromstring(ome_xml)
                    ns = {'ome': 'http://www.openmicroscopy.org/Schemas/OME/2016-06'}
                    pixels_node = root.find('.//ome:Pixels', ns)
                    if pixels_node is not None:
                        phys_x = pixels_node.get('PhysicalSizeX')
                        phys_y = pixels_node.get('PhysicalSizeY')
                        unit_x = pixels_node.get('PhysicalSizeXUnit')
                        if phys_x and phys_y and phys_x == phys_y and unit_x == 'µm':
                             if pixel_size_um is None:
                                 pixel_size_um = float(phys_x)
                                 print(f"  Read pixel size from OME metadata: {pixel_size_um:.4f} um/pixel")
                             else: print("  Pixel size already set, ignoring OME pixel size.")
                        if frame_interval_sec is None:
                            ...
                            if plane_node is not None and 'DeltaT' in plane_node.attrib:
                                delta_t = float(plane_node.attrib['DeltaT'])
                                delta_t_unit = plane_node.attrib.get('DeltaTUnit', 's')
                                if delta_t_unit == 'ms': frame_interval_sec = delta_t / 1000.0
                                elif delta_t_unit == 's': frame_interval_sec = delta_t
                                else: print(f"  Warning: Unknown OME DeltaTUnit '{delta_t_unit}', assuming seconds."); frame_interval_sec = delta_t
            ...
            # <<< ADDED: Check standard TIFF resolution tags >>>
            if pixel_size_um is None:
                 try:
                     # Need to access tags via the TiffPage object
                     page = tif.pages[0] # Assuming metadata is on first page
                     x_res_tag = page.tags.get(282) # XResolution Tag ID
                     y_res_tag = page.tags.get(283) # YResolution Tag ID
                     unit_tag = page.tags.get(296)  # ResolutionUnit Tag ID

                     if x_res_tag and y_res_tag and unit_tag:
                         # Resolution is often stored as a fraction (numerator, denominator)
                         x_res_num, x_res_den = x_res_tag.value
                         y_res_num, y_res_den = y_res_tag.value
                         unit = unit_tag.value

                         # Ensure denominator is not zero and resolutions match for square pixels
                         if x_res_den != 0 and y_res_den != 0 and (x_res_num / x_res_den) == (y_res_num / y_res_den):
                             ...
                             if pixels_per_unit > 0:
                                 ...
                                 if res_unit_um is not None:
                                     pixel_size_um = res_unit_um / pixels_per_unit
                                     print(f"  Read pixel size from standard TIFF tags (Unit={unit_name}): {pixel_size_um:.4f} um/pixel")
                                 else:
                                     print(f"  Found X/YResolution tags but ResolutionUnit ({unit}) is not Inch or Centimeter.")
                         else:
                             print("  Found X/YResolution tags but pixels are not square or resolution is zero.")
                     else:
                         print("  Standard TIFF resolution tags (XRes, YRes, Unit) not found or incomplete.")

                 except Exception as e:
                     print(f"Could not process standard TIFF resolution tags: {e}")
            # <<< END ADDED >>>
    ...
    if pixel_size_um is None:
        print("\nWarning: Pixel size (um/pixel) could not be read from metadata.")
        print("         Calculations and plots will use PIXEL units for area.")
        print(f"         To use micrometers, set 'cfg_pixel_size_um' manually in the script's CONFIGURATION section.")
        area_per_pixel_um2 = None
        area_units_label = "pixels"
        area_col_name = "area_pixels"
    else:
        print(f"Using pixel size: {pixel_size_um:.4f} um/pixel")
        area_per_pixel_um2 = pixel_size_um * pixel_size_um
        area_units_label = "um^2"
        area_col_name = "area_um2"
    ...
    if frame_interval_sec is not None and frame_interval_sec > 0:
        ...
        print(f"Using frame interval: {frame_interval_sec:.4f} sec (Rate: {frame_rate:.3f} FPS)")
    else:
    ...
    param_add_opening = cfg_add_opening
    param_opening_radius = cfg_opening_disk_radius
    param_add_closing = cfg_add_closing
    param_closing_radius = cfg_closing_disk_radius
    ...
    initial_params = {
        ...
        'add_opening': param_add_opening,
        'opening_disk_radius': param_opening_radius,
        'add_closing': param_add_closing,
        'closing_disk_radius': param_closing_radius
    }

    approved_params = interactive_parameter_tuning(image_stack, fak_channel_index, num_frames, initial_params)

    # Update local variables with approved parameters
    param_segmentation_method = approved_params['segmentation_method']
    ...
    param_add_opening = approved_params['add_opening']
    param_opening_radius = approved_params['opening_disk_radius']
    param_add_closing = approved_params['add_closing']
    param_closing_radius = approved_params['closing_disk_radius']
    ...
    print(f" Max FA Area: {param_max_fa_area if param_max_fa_area is not None else 'None'}")
    ...
    print(f" Use Opening: {param_add_opening} (Radius: {param_opening_radius})")
    print(f" Use Closing: {param_add_closing} (Radius: {param_closing_radius})")
    print("--------------------------------")
    ...
    if not preview_ok:
        print("Analysis aborted after segmentation preview.")
        sys.exit()
    # --- >>> End Preview Step <<< ---
    ...
    for t in range(num_frames):
        ...
        if num_labels > 0:
            ...
            features_df.rename(columns={'mean_intensity': 'fak_intensity_mean',
                                        'max_intensity': 'fak_intensity_max',
                                        'area': 'area_pixels'}, inplace=True) # Rename area column

            features_df['fak_intensity_integrated'] = features_df['fak_intensity_mean'] * features_df['area_pixels']
            ...
            try:
                 ...
                 features_df['vimentin_intensity_integrated'] = features_df.apply(
                     lambda row: row['vimentin_intensity_mean'] * row['area_pixels'] if pd.notna(row['vimentin_intensity_mean']) else np.nan,
                     axis=1
                 )
            except Exception as e:
                 ...
                 features_df['vimentin_intensity_integrated'] = np.nan

            # Calculate area in um^2 if pixel size is known
            if area_per_pixel_um2 is not None:
                features_df['area_um2'] = features_df['area_pixels'] * area_per_pixel_um2
            else:
                features_df['area_um2'] = np.nan # Or keep only pixel area column
    ...
    if tracks_filtered.empty:
        ...
        # Save unfiltered for inspection
        ...
        param_log_path = os.path.join(output_dir, f"{output_prefix}_parameters.json")
        print(f"Saving analysis parameters to {param_log_path}...")
        analysis_parameters = {
            'Input File': tiff_path, 'Output Directory': output_dir, 'Output Prefix': output_prefix,
            'Vimentin Channel Index': vimentin_channel_index, 'FAK Channel Index': fak_channel_index,
            'Pixel Size (um/pixel)': pixel_size_um if pixel_size_um is not None else "Not Found/Set",
            'Frame Interval (sec)': frame_interval_sec, 'Frame Rate (FPS)': frame_rate,
            '--- Segmentation ---': '--- TUNED PARAMETERS ---',
            'Segmentation Method': param_segmentation_method, 'Use Gaussian Blur': param_use_gaussian_blur,
            'Gaussian Sigma': param_gaussian_sigma,
            'Local Block Size': param_local_block_size if param_segmentation_method == 'local' else 'N/A',
            'Local Offset': param_local_offset if param_segmentation_method == 'local' else 'N/A',
            'Local Method': param_local_method if param_segmentation_method == 'local' else 'N/A',
            'Manual Threshold': param_manual_threshold_value if param_segmentation_method == 'manual' else 'N/A',
            '--- Cleaning ---': '--- TUNED PARAMETERS ---',
            'Min FA Area (pixels)': param_min_fa_area,
            'Max FA Area (pixels)': param_max_fa_area,
            'Remove Border Objects': param_remove_border_objects,
            'Use Opening': param_add_opening, 'Opening Radius': param_opening_radius,
            'Use Closing': param_add_closing, 'Closing Radius': param_closing_radius,
            '--- Tracking ---': '--- FIXED PARAMETERS ---',
            'Search Range (pixels)': cfg_search_range, 'Memory (frames)': cfg_memory,
            '--- Filtering ---': '--- FIXED PARAMETERS ---',
            'Min Track Length (frames)': cfg_min_track_length
        }
        try:
            with open(param_log_path, 'w') as f: json.dump(analysis_parameters, f, indent=4)
            print(f"✅ Parameters successfully saved.")
        ...
        try:
            output_columns = [
                'frame', 'particle', 'label', 'y', 'x', 'area_pixels', 'area_um2', # Added area_um2
        ...
        try:
            # ... (trajectory plotting loop as before) ...
            ...
            fig_traj.savefig(traj_plot_path, dpi=150)
        ...
        if num_plots_to_show > 0:
             ...
             for i in range(num_plots_to_show):
                  ...
                  fig_ind, axes = plt.subplots(5, 1, figsize=(10, 12), sharex=True)
                  ...
                  # Plot Area (use correct column and label)
                  area_col = 'area_um2' if area_per_pixel_um2 is not None and 'area_um2' in track_data.columns else 'area_pixels'
                  axes[4].plot(time_sec, track_data[area_col], 'b.-', label='Area')
                  axes[4].set_ylabel(f'Area ({area_units_label})') # Use dynamic label
