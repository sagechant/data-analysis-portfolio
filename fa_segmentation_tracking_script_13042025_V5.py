...
# <<< Import specific morphology elements if needed >>>
from skimage.morphology import disk # Needed for binary_opening/closing footprint
...
output_dir = r'C:/Users/Arun/Downloads/PhD/Manuscript/TIRF LIVE IMAGING/Python Analysis Results/Trial'
...
cfg_memory = 6               # <<< Increased Memory Significantly >>> Allows bridging up to 5 missed frames
...
def clean_mask(mask, min_area=10, max_area=None, remove_border=True):
    ...
    cleaned_mask = mask
    if min_area > 0:
        # Check if mask is not empty before removing small objects
        if np.any(cleaned_mask):
            cleaned_mask = morphology.remove_small_objects(cleaned_mask, min_size=min_area)
        else:
            return cleaned_mask # Return empty mask if already empty

    # Morphological opening to remove small protrusions/noise
    if np.any(cleaned_mask):
        cleaned_mask = morphology.binary_opening(cleaned_mask, footprint=disk(1))
    else:
         return cleaned_mask # Return empty mask if opening made it empty

    # <<< ADDED: Morphological closing to fill small holes >>>
    if np.any(cleaned_mask):
        # Use a slightly larger footprint for closing if needed, e.g., disk(2)
        cleaned_mask = morphology.binary_closing(cleaned_mask, footprint=disk(2))
    else:
        return cleaned_mask # Return empty mask if closing made it empty
    # <<< END ADDED >>>


    if max_area is not None and max_area > 0:
         labels, num_labels = measure.label(cleaned_mask, return_num=True)
         if num_labels > 0:
...
def interactive_parameter_tuning(full_image_stack, fak_ch_index, num_total_frames, initial_params):
    ...
    print("Use 'Key Frame' slider to switch preview (0=Start, 1=Mid, 2=End).")
...
# --- MAIN SCRIPT ---
# [main function remains largely the same, calling the updated tuner and preview]
# [processing loop, tracking, filtering, saving, plotting remain the same]
# [Omitted here for brevity, but it's the same as the previous version's main function]
def main():
