...
# <<< MODIFIED: Accepts full stack and channel index >>>
def interactive_parameter_tuning(full_image_stack, fak_ch_index, num_total_frames, initial_params):
    ...
    while True:
        ...
        # Only update if values changed (optional optimization)
        ...
        if frame_idx != last_frame_idx or params_changed:
            try:
                ...
                # Normalize current frame for display
                ...
                # Display a blank or error message image? For now, just skip update.
                # Need to handle how to display errors without crashing the loop.
                # Let's just show the last successful preview or a blank screen.
                # Create a blank image with error text
                err_img = np.zeros((display_img_bgr.shape[0], display_img_bgr.shape[1], 3), dtype=np.uint8)
                ...
                preview_img = err_img # Show error image
...
def main():
    ...
    # Read initial config values into local scope for clarity before tuning
    ...
    # <<< MODIFIED: Pass full stack, channel index, and frame count >>>
    ...
    for t in range(num_frames):
        ...
        # Extract features for labeled objects if any were found
        if num_labels > 0:
    ...
    if tracks_filtered.empty:
        ...
        if num_plots_to_show > 0:
             ...
             for i in range(num_plots_to_show):
                  ...
                  # Plot Vimentin, FAK, Area... [omitted for brevity, same as before]
